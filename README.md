...
The expressions can be called as a univariate function, `u(x)`, a univariate function with parameters, `u(x, p)`, or as a bivariate function, `u(x,y)` (with `y` being a parameter). All typical calling patterns when a function is passed to a numeric routine. These calls substitute in for the symbolic value (and parameter) when not specified as `nothing`. (To substitute in for just the parameter, either `u(nothing, value)` *or* `u(:,value)`.)
